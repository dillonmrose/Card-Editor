...
public class CardEditorDriver {
	...
	private static HashMap<String, HashMap<String, List<Point>>> getExpansionCode2TBPointsMap(
		...
		for (String line = br.readLine(); line != null; line = br.readLine()) {
			...
			TBPoints.add(new Point(Integer.parseInt(lineSplit[2]), Integer.parseInt(lineSplit[3])));
			TBPoints.add(new Point(Integer.parseInt(lineSplit[4]), Integer.parseInt(lineSplit[5])));
			TBPoints.add(new Point(Integer.parseInt(lineSplit[6]), Integer.parseInt(lineSplit[7])));
	...
	private static void boozifyImage(String baseImageFile,
			String boozeImageFile, String boozeText, String expansion_code, String type) throws Exception {
	...
	private static void clearTextBox(Graphics2D ig2,
			BufferedImage bi, List<Point> TBPoints) {
		int TLX = (int) TBPoints.get(0).getX();
		int TLY = (int) TBPoints.get(0).getY();
		int BRX1 = (int) TBPoints.get(1).getX();
		int BRY1 = (int) TBPoints.get(1).getY();
